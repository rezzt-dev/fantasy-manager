...
import { a as buildTeamStrength, d as buildShrinkagePriors, f as buildTeamTiers, i as buildPositionAverages, n as fetchTeamElos, p as isSuspended, r as fetchAvailableFormations, s as estimatePointsDetailed, t as fetchProbableLineups, y as combinedSignal } from "./jornadaperfecta__8wHAFaK.mjs";
...
*/
var SCORE_DIR = nodePath.join(process.cwd(), "data", "score-predictions");
function snapshotFile(leagueId, week) {
	return nodePath.join(SCORE_DIR, `${leagueId}-w${week}.json`);
}
var HISTORY_FILE = nodePath.join(SCORE_DIR, "history.jsonl");
...
async function appendScorePredictionHistory(response) {
	...
	await withFileLock(HISTORY_FILE, async () => {
		...
		for (const record of records) byKey.set(`${record.leagueId}:${record.week}:${record.teamId}`, record);
