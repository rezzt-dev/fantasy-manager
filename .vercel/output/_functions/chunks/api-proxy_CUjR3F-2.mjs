...
function isTokenExpired(token) {
	try {
		...
		return payload.exp * 1e3 - Date.now() < 3e5;
	} catch {
		return true;
	}
}
async function performTokenRefresh(refreshTokenValue) {
	...
	if (!data.id_token && !data.access_token) throw new Error("Refresh token response missing access tokens");
	return {
...
async function readSession(session) {
	try {
		return await session.get("fantasy_tokens");
	} catch (error) {
...
async function writeSession(session, tokens) {
	try {
...
async function getOrRefreshTokens(cookies, session) {
	let tokens;
	if (session) tokens = await readSession(session);
	if (!tokens) {
		const tokensRaw = cookies.get("fantasy_tokens")?.value;
		if (tokensRaw) try {
			tokens = JSON.parse(decodeURIComponent(tokensRaw));
		} catch {}
	}
	if (!tokens) return getEnvOptional("LALIGA_FANTASY_TOKEN");
	if (tokens.access_token && isTokenExpired(tokens.access_token)) {
		if (tokens.refresh_token) try {
			...
			if (session) await writeSession(session, newTokens);
			if (cookies.set) cookies.set("fantasy_tokens", encodeURIComponent(JSON.stringify(newTokens)), {
				httpOnly: true,
				path: "/",
...
*/
function normalizeFormation(lineup) {
...
function countFieldPlayers(lineup) {
	const f = lineup?.formation;
	if (!f) return 0;
	return (f.goalkeeper?.length || 0) + (f.defender?.length || 0) + (f.midfielder?.length || 0) + (f.attacker?.length || 0);
}
/**
...
*/
async function fetchCurrentLineup(token, teamId, weekNumber, _leagueId) {
	let genericLineup = null;
	let weekLineup = null;
	try {
		genericLineup = normalizeFormation(await fetchOfficialAPI(`${CMP}/teams/${teamId}/lineup`, token));
	} catch (error) {
		console.warn("[fetchCurrentLineup] generic team lineup failed:", error instanceof Error ? error.message : error);
	}
	try {
		weekLineup = normalizeFormation(await fetchOfficialAPI(`${CMP}/teams/${teamId}/lineup/week/${weekNumber}`, token));
	} catch (error) {
	...
	if (weekCount > 0 && weekCount >= genericCount) return weekLineup;
	if (genericCount > 0) return genericLineup;
...
//#endregion
export { normalizeFormation as a, getEnvOptional as c, getToken as i, fetchCurrentLineup as n, readSession as o, fetchOfficialAPI as r, getEnv as s, CMP as t };
