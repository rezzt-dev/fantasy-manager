import { c as getEnvOptional, r as fetchOfficialAPI, t as CMP } from "./api-proxy_CUjR3F-2.mjs";
import { o as estimatePoints, p as isSuspended, s as estimatePointsDetailed, u as resolveTeamId, y as combinedSignal } from "./jornadaperfecta__8wHAFaK.mjs";
import { createStorage } from "unstorage";
import upstashDriver from "unstorage/drivers/upstash";
import fsLiteDriver from "unstorage/drivers/fs-lite";
//#region src/lib/recommendations/captain.ts
var COACH_POSITION_ID$1 = 5;
...
*/
function recommendCaptain(analysis, estimatorContext, options) {
	...
	for (const tp of teamData?.players || []) universe.set(tp.playerMaster.id, tp.playerMaster);
	for (const entry of currentEntries) if (!universe.has(entry.playerMaster.id)) universe.set(entry.playerMaster.id, entry.playerMaster);
	const evaluated = [...universe.values()].filter((player) => player.positionId !== COACH_POSITION_ID$1).map((player) => buildCandidate(player, {
		estimatorContext,
		externalSignals,
		starterInfo,
		calendar,
	...
	if (evaluated.length === 0) return void 0;
	...
	const { pool, ranked } = currentPool.length > 0 ? {
		pool: "lineup",
		ranked: currentPool
		...
		pool: "squad",
		ranked: [...evaluated].sort(byScore)
	};
	const captain = pickBest(ranked);
	if (!captain) return void 0;
	const alternatives = ranked.filter((c) => c.player.id !== captain.player.id).slice(0, MAX_ALTERNATIVES);
	const bestAlternative = alternatives[0];
	const gainOverAlternative = round1$1(bestAlternative ? Math.max(0, captain.expectedPoints - bestAlternative.expectedPoints) : captain.expectedPoints);
	const optimalBest = pickBest(optimalPool);
	return {
		captain,
		alternatives,
		gainOverAlternative,
		optimalCaptain: optimalBest && optimalBest.player.id !== captain.player.id ? optimalBest : void 0,
		pool,
...
function pickBest(ranked) {
	return ranked.find((c) => c.hasFixture && c.isHealthy) ?? ranked.find((c) => c.hasFixture) ?? ranked[0];
}
function buildCandidate(player, input) {
	...
	const expectedPoints = round1$1(prediction.xp);
	const floorPoints = round1$1(prediction.riskAdjustedXp);
	const teamId = resolveTeamId(player);
	const homeMatch = teamId !== void 0 ? calendar.find((m) => m.localId === teamId) : void 0;
	const awayMatch = teamId !== void 0 ? calendar.find((m) => m.visitorId === teamId) : void 0;
	const hasFixture = calendar.length === 0 || teamId === void 0 || Boolean(homeMatch || awayMatch);
	...
	const hasBadNews = external.signal === "sell" && external.confidence >= BAD_NEWS_CONFIDENCE$1;
	const starter = starterInfo?.[player.id];
	const score = EV_WEIGHT * prediction.xp + .30000000000000004 * prediction.riskAdjustedXp;
	const risks = [];
	...
	if (prediction.pStarter !== null && prediction.pStarter < ROTATION_RISK_PSTARTER) risks.push(`Solo ${Math.round(prediction.pStarter * 100)}% de probabilidad de ser titular.`);
	if (prediction.dataQuality.level === "low") risks.push("Pocos datos para estimar su puntuación con confianza.");
	return {
		player,
		expectedPoints,
		floorPoints,
		captainBonus: expectedPoints,
		totalWithArmband: round1$1(prediction.xp * 2),
		score: round2(score),
...
function buildReasoning(input) {
	...
	const parts = [`${expectedPoints.toFixed(1)} pts esperados (${(expectedPoints * 2).toFixed(1)} con brazalete)`];
...
function buildCandidates(input) {
	...
	const pool = canFillAnyFormation(healthy, formations) ? healthy : all.filter((c) => !isSuspended(c.player, context?.injuryReport) && (c.source === "squad" || isHealthy(c.player, context)));
	for (const c of pool) {
...
*/
var upstashUrl = getEnvOptional("UPSTASH_REDIS_REST_URL");
var upstashToken = getEnvOptional("UPSTASH_REDIS_REST_TOKEN");
var storage = createStorage({ driver: upstashUrl && upstashToken ? upstashDriver({
	url: upstashUrl,
	token: upstashToken
...
*/
var warnedOperations = /* @__PURE__ */ new Set();
function warnOnce(operation, key, error) {
...
async function kvSet(key, value, ttlSeconds) {
	try {
		await storage.setItem(key, value, ttlSeconds ? { ttl: ttlSeconds } : void 0);
	} catch (error) {
...
var CACHE_TTL_S = CACHE_TTL_MS / 1e3;
var KV_PREFIX = "player-stats";
var CONCURRENCY = 5;
var memCache = /* @__PURE__ */ new Map();
async function readKvCache(playerId) {
	const entry = await kvGet(`${KV_PREFIX}:${playerId}`);
	if (!entry || !Array.isArray(entry.stats)) return null;
	if (Date.now() - entry.fetchedAt > CACHE_TTL_MS) return null;
	return entry.stats;
}
async function writeKvCache(playerId, stats) {
	const entry = {
		fetchedAt: Date.now(),
		stats
	};
...
//#endregion
export { kvSet as a, fetchLeagueActivity as c, computeAvailableBudget as d, computeFormationFronts as f, recommendCaptain as h, kvGet as i, marketFlowByManager as l, buildLeagueAnalysis as m, starterScoreFromLastSeason as n, analyzeClauseRisks as o, computeTacticalScheme as p, fetchPlayerStats as r, rivalSpendingPower as s, fetchStarterInfo as t, getClauseProtection as u };
