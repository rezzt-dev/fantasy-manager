...
import { _ as loadEngineParams, a as buildTeamStrength, b as fetchExternalSignals, c as estimatePointsLegacy, d as buildShrinkagePriors, f as buildTeamTiers, g as getEngineParams, h as DEFAULT_ENGINE_PARAMS, i as buildPositionAverages, l as predictPlayerPoints, n as fetchTeamElos, o as estimatePoints, r as fetchAvailableFormations, s as estimatePointsDetailed, t as fetchProbableLineups, u as resolveTeamId, v as saveEngineParams, y as combinedSignal } from "./jornadaperfecta__8wHAFaK.mjs";
import { a as kvSet, c as fetchLeagueActivity, d as computeAvailableBudget, f as computeFormationFronts, h as recommendCaptain, i as kvGet, m as buildLeagueAnalysis, n as starterScoreFromLastSeason, o as analyzeClauseRisks, p as computeTacticalScheme, r as fetchPlayerStats, t as fetchStarterInfo, u as getClauseProtection } from "./starter-status_qu3Tyyay.mjs";
...
import { a as persistMetrics, c as persistScoringTable, i as persistLineup, l as settleTrackRecord, n as evaluateTrackRecord, o as persistPredictions, r as evaluateWalkForward, s as persistRecommendations, t as MODEL_VERSION } from "./track-record_BhHjzl7V.mjs";
...
function generateRecommendations(input) {
	...
	if (league?.config?.premiumFeatures?.captain !== false && captain?.captain) {
		...
		const details = [captain.alternatives.length > 0 ? `Alternativas: ${captain.alternatives.map((a) => `${a.player.nickname} (${a.expectedPoints.toFixed(1)})`).join(", ")}.` : "No hay alternativas claras.", ...cap.risks].join(" ");
		recommendations.push({
...
* sin dependencias.
*/
var SNAPSHOT_DIR = nodePath.join(process.cwd(), "data", "snapshots");
...
async function maybeWriteDailySnapshot(input) {
	try {
		...
		const file = nodePath.join(SNAPSHOT_DIR, `${date}.json`);
...
var CALIBRATION_FILE = nodePath.join(process.cwd(), "data", "track-record", "calibration-latest.json");
...
async function persistCalibration(result) {
	await mkdir(nodePath.dirname(CALIBRATION_FILE), { recursive: true });
...
async function fetchCalendarCached(week, currentWeek, token) {
	...
	let matches;
	try {
		matches = await fetchOfficialAPI(`${CMP}/calendar`, token, { weekNumber: String(week) });
	} catch (error) {
		console.warn(`[calendar-cache] week ${week} fetch failed:`, error instanceof Error ? error.message : error);
		return [];
	}
	if (Array.isArray(matches) && matches.length > 0) {
		await kvSet(`${KV_PREFIX}:w${week}`, {
			fetchedAt: Date.now(),
			matches
		}, week === currentWeek ? CURRENT_TTL_MS / 1e3 : void 0);
		return matches;
	}
	return [];
}
