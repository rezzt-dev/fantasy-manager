...
import { S as createAstro, h as addAttribute, i as renderComponent, l as renderTemplate, m as renderHead, p as maybeRenderHead } from "./server_mK3GCZoW.mjs";
...
var $$BenefitCard = createComponent(($$result, $$props, $$slots) => {
	...
	return renderTemplate`${maybeRenderHead($$result)}<div class="rounded-2xl border border-white/[0.06] bg-surface p-6"><div class="flex h-10 w-10 items-center justify-center rounded-lg border border-white/[0.08] bg-surface-2">${renderComponent($$result, "Icon", Icon, { "className": "h-5 w-5 text-foreground" })}</div><h3 class="mt-4 font-display text-base font-semibold tracking-tight text-foreground">${title}</h3><p class="mt-2 text-sm leading-relaxed text-muted-foreground">${description}</p></div>`;
...
var $$StepCard = createComponent(($$result, $$props, $$slots) => {
	...
	return renderTemplate`${maybeRenderHead($$result)}<div class="rounded-2xl border border-white/[0.06] bg-surface-2 p-6"><div class="flex items-center justify-between"><div class="flex h-10 w-10 items-center justify-center rounded-lg border border-white/[0.08] bg-surface-3">${renderComponent($$result, "Icon", Icon, { "className": "h-5 w-5 text-foreground" })}</div><span class="font-display text-sm font-semibold tracking-widest text-brand-subtle">${step}</span></div><h3 class="mt-4 font-display text-base font-semibold tracking-tight text-foreground">${title}</h3><p class="mt-2 text-sm leading-relaxed text-muted-foreground">${description}</p></div>`;
...
var buttonVariants = cva("inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:size-4 [&_svg]:shrink-0", {
	variants: {
		variant: {
			default: "bg-brand text-background hover:bg-white",
...
var $$Index = createComponent(($$result, $$props, $$slots) => {
	...
	}, { "default": ($$result) => renderTemplate`Empezar${renderComponent($$result, "ArrowRight", ArrowRight, { "className": "h-4 w-4" })}` })}</a></div></div></header><main class="flex-1"><!-- Hero --><section class="relative flex min-h-screen flex-col items-center justify-center px-4 pt-24 pb-16 sm:pb-24"><div class="container max-w-4xl text-center"><div class="mb-6 inline-flex items-center gap-2 rounded-full border border-white/[0.08] bg-surface-2 px-4 py-1.5 text-xs font-medium text-muted-foreground"><span class="h-1.5 w-1.5 rounded-full bg-brand"></span>Centro de comando para managers competitivos</div><h1 class="font-display text-4xl font-semibold tracking-tight text-foreground sm:text-5xl md:text-6xl lg:text-7xl">Deja de perder puntos por decidir a ciegas</h1><p class="mx-auto mt-6 max-w-2xl text-base leading-relaxed text-muted-foreground sm:text-lg">Conecta tu cuenta de LALIGA FANTASY y accede a recomendaciones claras de compra, venta, capitán y cláusulas antes de cada jornada. Todo tu equipo, tu mercado y tus rivales en un único dashboard.</p><div class="mt-9 flex flex-col items-center justify-center gap-3 sm:flex-row"><a href="/login">${renderComponent($$result, "Button", Button, {
		"size": "lg",
