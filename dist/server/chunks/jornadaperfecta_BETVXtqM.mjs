import { n as getEnvOptional } from "./env_F5WbBaSV.mjs";
import { n as fetchOfficialAPI } from "./api-proxy_CJ5fp98A.mjs";
import { n as recentForm } from "./form_3qfbgRvD.mjs";
import { r as fetchTextWithCache, t as buildTeamMatcher } from "./team-names_DwPbX-En.mjs";
...
*/
var PARAMS_FILE = path.join(process.cwd(), "data", "engine-params.json");
...
function predictPlayerPoints(player, matches, context) {
	...
	if (evidence.hardNegative >= BAD_NEWS_CONFIDENCE$1) base *= BAD_NEWS_MULTIPLIER;
	else if (evidence.doubt >= .5) base *= .7;
	else if (evidence.rotation >= .5) base *= .85;
	if (evidence.hardNegative < BAD_NEWS_CONFIDENCE$1 && evidence.positive >= GOOD_NEWS_CONFIDENCE) base *= GOOD_NEWS_MULTIPLIER;
...
function computeOptimalLineup(input) {
	...
	const changes = currentEntries.length > 0 ? currentEntries.map((e) => e.playerMaster).filter((p) => !starterIds.has(p.id) && p.positionId !== COACH_POSITION_ID).sort((a, b) => a.positionId - b.positionId).map((out, i) => ({
		out,
		in: best.starters.map((e) => e.player).filter((p) => !currentIds.has(p.id)).sort((a, b) => a.positionId - b.positionId)[i] || out
	})) : [];
...
//#endregion
export { loadEngineParams as _, buildPositionAverages as a, fetchExternalSignals as b, estimatePointsDetailed as c, resolveTeamId as d, buildShrinkagePriors as f, getEngineParams as g, DEFAULT_ENGINE_PARAMS as h, computeOptimalLineup as i, estimatePointsLegacy as l, normalizePlayerName as m, fetchTeamElos as n, buildTeamStrength as o, buildTeamTiers as p, fetchAvailableFormations as r, estimatePoints as s, fetchProbableLineups as t, predictPlayerPoints as u, saveEngineParams as v, combinedSignal as y };
