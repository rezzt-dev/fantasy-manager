...
import { r as getToken } from "./api-proxy_CJ5fp98A.mjs";
import { u as summarizeTrackRecord } from "./track-record_BcljYC3i.mjs";
...
*/
var TRACK_RECORD_DIR = path.join(process.cwd(), "data", "track-record");
...
var GET = async ({ url, cookies, session }) => {
	try {
		...
		const metricsLatest = await readJson(path.join(TRACK_RECORD_DIR, "metrics-latest.json"));
